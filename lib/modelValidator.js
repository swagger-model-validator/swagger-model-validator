...
function Validator(swagger) {
    ...
    if(swagger) {
        swagger.validateModel = swaggerValidate;
    }
}
...
function swaggerValidate(modelName, obj) {
    ...
    return validate(obj, model, this.allModels);
}

Validator.prototype.validate = function(target, swaggerModel, swaggerModels, allowBlankTargets) {
    return validate(target, swaggerModel, swaggerModels, allowBlankTargets);
};

function validate(target, swaggerModel, swaggerModels, allowBlankTargets) {
    ...
    var validationErrs = validateSpec(target, swaggerModel.properties, swaggerModels);
...
function validateSpec(target, properties, models) {
    var errors = [];
    if(!properties) {
        return null;
    }
    ...
    if(expectedType === 'array') {
        if(field.items && field.items.type) {
            ...
            if(model) {
                ...
                var arrayErrors = [];
                property.forEach(function(value) {
                    var errors = validate(value, model.properties, models);
...
function validateMinMaxLength(name, value, minLength, maxLength) {
    ...
    if(value.length < minLength || value.length > maxLength){
        if(minLength === 1) {
            return new Error(name + ' cannot be blank and cannot be long than ' + maxLength.toString() + ' characters long');
        }
