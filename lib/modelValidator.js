    ...
    if(!expectedType) {
        if(models && field.$ref) {
            ...
            for(var of in oneOfArray) {
                ...
                var error = validateType(name, value, field, models, allowBlankTargets, disallowExtraProperties);
                if(error===null) {
                    return null;
                }
                errors.push(error);
    ...
    if(field.items && field.items.type) {
        ...
        if (field.type && field.uniqueItems) {
            ...
            for (var i = 0; i < value.length; i++) {
                var j = i + 1
                while(j < value.length && !doesArrayContainsValue(duplicatedObjects, value[j])) {
                    if (_.isEqual(value[i], value[j])) {
                        duplicatedObjects.push(value[i])
                    }
                    j++;
                }
            }
...
function doesArrayContainsValue(arr, value) {
    return arr.find(val => _.isEqual(value, val)) != null
}
