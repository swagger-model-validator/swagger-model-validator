    ...
    if(value !== undefined) {
        ...
        if(field.type === 'integer' || field.type === 'number' || field["x-validatedType"] === 'date' || field["x-validatedType"] === 'date-time') {
            ...
            if (field.exclusiveMinimum || field.exclusiveMaximum || field.exclusiveMinimum === 0 || field.exclusiveMaximum === 0) {
                var err3 = validateMinMaxValue(key, value, field.exclusiveMinimum, field.exclusiveMaximum, true);
...
function validateMinMaxValue(name, value, minValue, maxValue, exclusive) {
    if(!minValue && !maxValue && !minValue === 0 && !minValue === 0) {
        ...
        return validateMaxValue(name, value, maxValue, exclusive);
    } else if(maxValue === undefined) {
        return validateMinValue(name, value, minValue, exclusive);
    }

    if(!exclusive && (value < minValue || value > maxValue)) {
        return new Error(name + ' must be at least ' + minValue.toString() + ' and no more than ' + maxValue.toString());
    } else if(exclusive && (value <= minValue || value >= maxValue)) {
